...
CONTROL_AREA_PERC_THRESHOLD = 0.25
...
liveuamap_any_control <- monthly_data_sel |>
...
monthly_data_sel <- monthly_data_sel |>
                      ...
                      merge(liveuamap_any_control,by=c("ADM3_EN"),
                            all=TRUE) |>
                      # add a flag for whether the adm3 area is under ISIS control that month,
...
# 14112 rows, which = 294 (# ADM3 areas) * 2 (# seasons) * 24 (# years)
seasonstart_evi_nomask |>
  select(c(plant_season_year, ADM3_EN)) |>
  distinct() |>
  nrow()
# confirming below =  14112
length(unique(seasonstart_evi_nomask$ADM3_EN)) * 2 * 24

#294 rows
seasonstart_evi_nomask |>
  select(c(plant_season_year, ADM3_EN)) |>
  group_by(ADM3_EN) |>
  summarize(n_seasons=n_distinct(plant_season_year)) |>
  ungroup() |>
  # 48 season-years
  arrange(n_seasons)
...
# 21462 rows of this data
seasonal_data_productivity_init |>
  select(c(plant_season_year, ADM3_EN)) |>
  distinct() |>
  nrow()
...
seasonal_data_productivity_init |>
  ...
  summarize(n_seasons=n_distinct(plant_season_year)) |>
  ungroup() |>
  # 73 season-years - includes "non-growing" parts of the year,
  # accounting for increase in rows from the seasonstart_evi_nomask
  arrange(n_seasons)
...
seasonal_data_productivity <- seasonal_data_productivity_init |>
  ...
  mutate(evi_prod_scaled = max_evi_nomask_scaled - seasonstart_mean_evi_scaled,
         occ_status_lm = ifelse(grepl("During",occ_status_lm),
                                  'During',
                                  ifelse(grepl('Post',occ_status_lm),
                                         'Post',
                                         ifelse(grepl('Pre',occ_status_lm),
                                                'Pre',NA)))) |>
...
# and confirm this number matches the number of rows in the
# seasonal_data_productivity df
...
seasonal_data_productivity |>
  ...
  summarize(n_seasons=n_distinct(plant_season_year)) |>
  ungroup() |>
  # 72 season-years - includes "non-growing" parts of the year,
  # but removes WheatBarley-2000, so one fewer per adm3 area than
  # the seasonal_data_productivity_init
  arrange(n_seasons)
...
write.csv(seasonal_data_productivity,
          "data/combined/plant_season_liveuamap_adm3_data.csv",
          row.names=FALSE)
